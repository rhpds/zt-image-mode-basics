...
==== _Click on the `VM SSH session` tab._
...
Let's take a last look at `bootc status` to see what changed.
...
Now we have a `rollback` section that details the image we installed and the newly applied update image. 
